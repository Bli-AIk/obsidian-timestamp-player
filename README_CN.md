...
本 fork 尽量保留下方原 README 内容，便于对照。此区域之后的章节描述的是上游/旧版行为，即裸 `MM:SS` 和说话人行时间戳默认会被识别。本 fork 改为优先使用显式标记，并且默认关闭裸时间戳识别。
...
- **可选节拍器：** 可通过全局默认设置开启，或在区段中使用 `metronome=on`，播放时提供轻量 click 声和可视节拍脉冲。
