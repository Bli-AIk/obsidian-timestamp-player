...
在阅读视图下，每个显式时间戳标记会变成可点击的 `▶ 00:27` 按钮。点击从该位置播放，再次点击暂停。
...
- **小节/拍位时间戳** — 配置 BPM 后，`{b:4.3}` 可以按音乐位置跳转
- **分区节奏配置** — `{music bpm=135 delay=0.3 meter=4/4}` 配置当前音频区段中后续标记的节拍换算
...
裸时间戳（例如 `00:27`）和说话人行（例如 `Alice 00:27`）默认关闭，以避免误识别。可在插件设置中开启 **Recognize bare timestamps**（识别裸时间戳）来恢复原行为。
...
| 时间戳 | 对应音频 |
|--------|----------|
| `{t:00:27}`、`{t:01:02}` | 访谈录音-上半场.mp3 |
| `{t:00:15}`、`{t:00:45}` | 访谈录音-下半场.mp3 |

各区段完全独立——不同区段的时间戳可以重叠（例如都有 `{t:00:00}`）不会冲突。切换区段时，前一个音频会自动暂停。
...
3. 在 vault 中放入一个真实的音频文件 `music.ogg`，或把下面的嵌入文件名改成你的音频文件名。
4. 在 设置 → 第三方插件 中启用 **Timestamp Player**。
5. 在阅读视图打开包含以下内容的笔记：
...
`{t:00:03}` 应只控制 `second.ogg`；前面的按钮仍应控制 `music.ogg`。
