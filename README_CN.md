...
本 fork 尽量保留下方原 README 内容，便于对照。此区域之后的章节描述的是上游/旧版行为，即裸 `MM:SS` 和说话人行时间戳默认会被识别。本 fork 改为优先/默认使用显式标记，并且默认关闭裸时间戳识别。

本 fork 完全由 AI 编写。作者已亲自验证，并且正在自己的工作流中使用它；它的定位是作者临时使用的实用工具，不是一个打磨完善、面向通用场景并长期维护的产品。

- **显式时间戳标记：** 使用 `{t:00:27}` 或 `{t:01:02:03}` 创建可点击的绝对时间戳。
- **小节/拍位标记：** 使用 `{b:bar.beat}`，例如 `{b:4.3}`，按音乐位置跳转。
- **节奏配置：** 在音频嵌入后放置 `{music bpm=135 delay=0.3 meter=4/4 metronome=off}`。它会应用到当前音频区段中后续的标记。
- **头部延迟语义：** `delay` 会从 beat time 中减去，因此解析出的拍位音频时间是 `beat time - delay`。
- **旧语法兼容：** 裸 `MM:SS` 和说话人行时间戳只有在设置中启用 **Recognize bare timestamps**（识别裸时间戳）后才可用。
- **可选节拍器和当前拍位显示：** 带 `bpm` 的区段会在音频播放器旁显示当前拍位。可通过全局默认设置开启节拍器，或在区段中使用 `metronome=on`，播放时提供轻量 click 声和可视节拍脉冲。

本地手动检验：

1. 在插件项目目录中运行 `npm install`，然后运行 `npm run build`。
2. 把 `main.js`、`styles.css`、`manifest.json` 复制到 `<vault>/.obsidian/plugins/timestamp-player/`。
3. 确保笔记引用的音频文件真实存在于 vault 中，例如 `![[music.ogg]]`。
4. 在 Obsidian 设置 → 第三方插件 中启用 **Timestamp Player**。
5. 用阅读视图打开笔记，并测试 `{t:00:05}`、`{b:2.1}` 等显式标记。
